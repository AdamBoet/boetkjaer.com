...
- **Self-referential dictionary entries**: CC-CEDICT sometimes lists a sense as just `variant of X[pinyin]` with no independent meaning of its own, when a real definition for that same tone exists in another entry. Skip the self-referential one; keep the real one. This applies to `front`/`pronunciation` just as much as to components — e.g. 馆's raw definitions are `variant of 館|馆[guan3]` and `building/shop/...` at the *same* tone; keeping both produces a duplicated `pronunciation` like `guǎn, guan3 / guǎn, guan3`, which is always a sign the self-referential one should have been dropped instead of kept alongside the real sense.
- **Surname reading listed first (a repeat offender — check this every time)**: CC-CEDICT capitalizes the pinyin of proper-noun senses, and for a lot of common characters the *surname* sense sorts before the common one in `hanzi_lookup.js`'s raw `definitions`/`components.once` output — e.g. `官` → `Guan1: surname Guan` before `guan1: official`; `昔` → `Xi1: surname Xi` before `xi1: former times`; `扁` → `Pian1: surname Pian` before `bian3: flat`. Picking the first-listed sense without checking is exactly how a component ends up glossed `(surname Guan)` on a card about a *building*. Rule: if a component or word's own most-relevant definition has a **lowercase**-pinyin (non-proper-noun) sense available anywhere in its `definitions` list, use that one — never the capitalized-pinyin surname sense — even when the surname sense happens to be listed first.
